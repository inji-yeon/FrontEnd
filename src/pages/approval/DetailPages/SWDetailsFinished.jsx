...
import './SWDetailsFinished.css';
import { useSelector, useDispatch } from 'react-redux';
import { callSWDetailsFinAPI } from '../../../apis/ApprovalAPICalls';
import { useNavigate, useLocation, useParams } from 'react-router-dom';

function SWDetailsFinished(){

    const dispatch = useDispatch();
    const navigate = useNavigate();
    const { approvalDocCode } = useParams(); // approvalDocCode 가져옴
    const [selectedSection, setSelectedSection] = useState("approval");
    const swDetails = useSelector((state) => state.approvalReducer);

    console.log("swDetails: ", swDetails);
    console.log("swDetails.employeeDTOs: ", swDetails.employeeDTOs);
    console.log("swDetails.referenceEmployeeDTOs: ", swDetails.referenceEmployeeDTOs);
    ...
    useEffect(() => {
        ...
        dispatch(callSWDetailsFinAPI({ approvalDocCode }));
        ...
        <>
            ...
            <div className="form_body">
                <div className="form_title_section">
                    <div className="form_title">소프트웨어 사용 신청서</div>
                </div>
                ...
                <div style={{ display: 'table-row' }}>
                        ...
                        <input
                            ...
                            type="text"
                            name="softwareTitle"
                            value={swDetails?.softwareUse?.softwareTitle}
                        />                        
                        </div>
                    </div>
                    ...
                    <div className="tableRow">
                        ...
                        <div className ="swB" style={{ display: 'table-cell' }}>
                            <input className="sw_input_box" 
                            type="text"
                            name="kindOfSoftware"
                            value={swDetails?.softwareUse?.kindOfSoftware}
                            />
                        </div>
                    </div>
                    ...
                    <div className="tableRow">
                        ...
                        <div className ="swB" style={{ display: 'table-cell' }}>
                            ...
                            type="text"
                            name="softwareReason"
                            value={swDetails?.softwareUse?.softwareReason}
                            />
                        </div>
                    </div>

                    <div style={{ display: 'table-row' }}>
                        <div className ="A" style={{ display: 'table-cell' }}>시행 일자</div>
                        <div className ="B" style={{ display: 'table-cell' }}>
                            <input type="date" id="sw_start_date"  className="inputbox" name="softwareStartDate" value={swDetails?.softwareUse?.softwareStartDate}/>
                        </div>
                    </div>
            ...
            {selectedSection === "approval" && ( // 조건부 렌더링
                <div className='selected_as_approval_line'>
                    <div className='selected_lines_for_on_leave'>
                            ...
                            <tbody>
                                {swDetails.employeeDTOs && swDetails.employeeDTOs.map((employee, index) => (
                                    <tr key={index}>
                                        ...
                                        <td className={`selected_status_op ${index % 2 === 0 ? 'even' : 'odd'}`}>
                                            ...
                                            {swDetails.additionalApprovalLines && swDetails.additionalApprovalLines.map((approvalLine) => {
                ...
                <div className='selected_as_view_line'>
                    <div className='selected_ref_for_on_leave'>
                        <table className='selected_ref_list_ol'>
                            <tbody>
                            {swDetails.referenceEmployeeDTOs && swDetails.referenceEmployeeDTOs.map((referenceEmployee, index) => (
                                <tr key={index}>
                                    ...
                                    <td className={`selected_ref_status_op ${index % 2 === 0 ? 'even' : 'odd'}`}>
                                        {/* approvalLines에서 해당 employeeCode에 해당하는 approvalProcessStatus 가져오기 */}
                                        {swDetails.approvalReferences && swDetails.approvalReferences.map((approvalRef) => {
            ...
            {selectedSection === "attached" && (
                ...
                <div className="attached_file_for_ol">
                    <table className='attached_file_list_ol'>
                        <tbody>
                        {swDetails && swDetails.approvalAttachedFiles && swDetails.approvalAttachedFiles.map((file, index) => (
...
export default SWDetailsFinished;