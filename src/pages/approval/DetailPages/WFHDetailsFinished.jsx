...
import { callWFHDetailsFinAPI } from '../../../apis/ApprovalAPICalls';
import { useNavigate, useLocation, useParams } from 'react-router-dom';

function WFHDetailsFinished(){

    const dispatch = useDispatch();
    const navigate = useNavigate();
    const { approvalDocCode } = useParams(); // approvalDocCode 가져옴
    const [selectedSection, setSelectedSection] = useState("approval");
    const wfhDetails = useSelector((state) => state.approvalReducer);

    console.log("wfhDetails: ", wfhDetails);
    console.log("wfhDetails.employeeDTOs: ", wfhDetails.employeeDTOs);
    console.log("wfhDetails.referenceEmployeeDTOs: ", wfhDetails.referenceEmployeeDTOs);
    ...
    useEffect(() => {
        ...
        dispatch(callWFHDetailsFinAPI({ approvalDocCode }));
            ...
            <div className="form_body">
                ...
                <div style={{ display: 'table-row' }}>
                        ...
                        <input
                            ...
                            type="text"
                            name="workTypeTitle"
                            value={wfhDetails?.workType?.workTypeTitle}
                        />                        
                        </div>
                    </div>
                    ...
                    <div style={{ display: 'table-row' }}>
                        ...
                        <div className ="B" style={{ display: 'table-cell' }}>
                            <input type="date" id="ov_working_date"  className="inputbox" name="workTypeStartDate" value={wfhDetails?.workType?.workTypeStartDate}/>
                        </div>
                    </div>
                    ...
                    <div style={{ display: 'table-row' }}>
                        ...
                        <div className ="B" style={{ display: 'table-cell' }}>
                            ...
                            name="workTypePlace" value={wfhDetails?.workType?.workTypePlace}/>
                        </div>
                    </div>
                    ...
                    <div style={{ display: 'table-row' }}>
                        ...
                        <div className ="B" style={{ display: 'table-cell' }}>
                            <input className="input_box" type="text" name="workTypeReason" value={wfhDetails?.workType?.workTypeReason}/>
                        </div>
                    </div>
                    {wfhDetails.additionalApprovalLines && wfhDetails.additionalApprovalLines.map((approvalLine, index) => {
            ...
            {selectedSection === "approval" && ( // 조건부 렌더링
                <div className='selected_as_approval_line'>
                    <div className='selected_lines_for_on_leave'>
                            ...
                            <tbody>
                                {wfhDetails.employeeDTOs && wfhDetails.employeeDTOs.map((employee, index) => (
                                    <tr key={index}>
                                        ...
                                        <td className={`selected_status_op ${index % 2 === 0 ? 'even' : 'odd'}`}>
                                            ...
                                            {wfhDetails.additionalApprovalLines && wfhDetails.additionalApprovalLines.map((approvalLine) => {
                ...
                <div className='selected_as_view_line'>
                    <div className='selected_ref_for_on_leave'>
                        <table className='selected_ref_list_ol'>
                            <tbody>
                            {wfhDetails.referenceEmployeeDTOs && wfhDetails.referenceEmployeeDTOs.map((referenceEmployee, index) => (
                                <tr key={index}>
                                    ...
                                    <td className={`selected_ref_status_op ${index % 2 === 0 ? 'even' : 'odd'}`}>
                                        {/* approvalLines에서 해당 employeeCode에 해당하는 approvalProcessStatus 가져오기 */}
                                        {wfhDetails.approvalReferences && wfhDetails.approvalReferences.map((approvalRef) => {
            ...
            {selectedSection === "attached" && (
                ...
                <div className="attached_file_for_ol">
                    <table className='attached_file_list_ol'>
                        <tbody>
                        {wfhDetails && wfhDetails.approvalAttachedFiles && wfhDetails.approvalAttachedFiles.map((file, index) => (
...
export default WFHDetailsFinished;